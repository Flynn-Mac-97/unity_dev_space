...
| `FlynnAnimationDriver` | Controller → Animator (`Speed`/`IsGrounded`/`FacingDir`); flips sprite, billboards Visual. Owns `animator.speed`. **Swing phase** API for the wrench: `BeginChargePose(idx)` starts the windup (clip paused), `UpdateChargePose(idx, charge)` scrubs the paused clip across windup frames 1→5 as charge builds (holds at 5) and live-swaps the tool, `ReleaseSwing(idx, speed)` resumes from the held windup frame and plays out at a charge-scaled speed (heavier = slower), `CancelSwing()` aborts to idle. While charging/releasing the locomotion speed/facing logic is suppressed so the freeze/scale survives. Also `TriggerAttack(idx)` (legacy, AnyState trigger path), `FaceWorldDirection(dir)`, `IsAttacking` (now works — attack states are tagged `Attack`). |
...
**`CustomCursor`** (Player): swaps the OS pointer for `Sprites/cursor_crosshair.png` via `Cursor.SetCursor` (set-once). Texture is Cursor-type, read/write, uncompressed; hotspot = centre.
...
States: idle/run/jump × front/back/side + `Attack_{Pick,Axe,Hammer,Wrench}` (each tagged `Attack`).
Params: `Speed`(f), `IsGrounded`(b), `FacingDir`(i 0=front/1=back/2=side), `Attack`(trigger), `AttackIndex`(i 1–4). AnyState→Attack on trigger+index (legacy `TriggerAttack` path). The wrench swing instead `Play`s the attack state directly to freeze/scale it (see `FlynnAnimationDriver`). Regenerate via **Flynn → Setup Animations**.
